...
const { AudioWorker } = require('../src/services/audioWorker');
const { MediaService, parseRange, mimeFromPath } = require('../src/media/mediaService');
...
test('AudioWorker updates loudness for valid jobs, rejects invalid jobs, and clears idle lock', async () => {
  ...
  const repo = {
    ...
    async findSoundByHash(hash) {
      return hash === 'h' ? { sound_hash: 'h', data_link: '/music/a.wav' } : null;
    },
    async updateLoudness(hash, value) {
      this.loudness = value;
    },
  };
  const jobs = [{ hash: 'h', file_path: '/music/a.wav' }, { hash: 'h' }, null];
...
test('AudioWorker reports not_found and failed analysis branches', async () => {
  const workerNotFound = new AudioWorker({
    redis: { popAudioProcessing: async () => ({ hash: 'missing', file_path: '/music/a.wav' }) },
  ...
  const workerFailed = new AudioWorker({
    redis: { popAudioProcessing: async () => ({ hash: 'h', file_path: '/music/a.wav' }) },
    repository: { findSoundByHash: async () => ({ data_link: '/music/a.wav' }) },
