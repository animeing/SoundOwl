...
const { analyzeLoudness, probeMetadata } = require('../src/audio/ffmpeg');
const { normalizeMetadata, normalizeMusicMetadata, readMetadata } = require('../src/audio/metadata');

test('analyzeLoudness parses ffmpeg volumedetect mean and max volume', async () => {
  const execFileImpl = (command, args, callback) => {
    assert.equal(command, 'ffmpeg');
    assert.deepEqual(args.slice(0, 2), ['-i', '/music/a.wav']);
    callback(null, '', 'mean_volume: -18.4 dB\nmax_volume: -1.2 dB');
  };

  assert.deepEqual(await analyzeLoudness('/music/a.wav', { execFileImpl }), {
    mean_volume: -18.4,
    max_volume: -1.2,
    raw: '\nmean_volume: -18.4 dB\nmax_volume: -1.2 dB',
  });
...
test('probeMetadata parses ffprobe JSON output', async () => {
  ...
  assert.deepEqual(await probeMetadata('/music/a.wav', { execFileImpl }), { format: { tags: { title: 'Song' } } });
});
...
test('probeMetadata rejects invalid ffprobe JSON output', async () => {
  ...
  await assert.rejects(probeMetadata('/music/a.wav', { execFileImpl }), SyntaxError);
});
...
test('normalizeMetadata provides getID3-equivalent tag fallbacks and filename title fallback', () => {
  ...
  const untagged = normalizeMetadata({ format: {}, streams: [] }, '/music/no-tag.wav');
...
test('normalizeMusicMetadata uses library tags, pictures, native fallback, and filename fallback', () => {
  ...
  assert.equal(parsed.lyrics, 'Lyrics');
...
test('readMetadata delegates to music-metadata parser and normalizes the result', async () => {
  const parsed = await readMetadata('/music/library.wav', {
    parseFile: async (filePath, options) => {
      assert.equal(filePath, '/music/library.wav');
