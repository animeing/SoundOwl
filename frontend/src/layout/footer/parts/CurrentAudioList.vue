<template>
    <sw-resize :class="audioFrameClass()" resize-direction='top-left' >
        ...
        <div :class="audioListClass()" id='current-audio-list' style='overflow-y: scroll;' >
            <button v-for="(item, index) in soundClips" :key="index" drag-item  @click.right.prevent="contextMenu(item)" :class='audioItemClass(item)' @click="click(item)" class="draggable-item">
                <SoundClipComponentVue :sound-clip='item'></SoundClipComponentVue>
            </button>
	    </div>
    </sw-resize>
</template>
