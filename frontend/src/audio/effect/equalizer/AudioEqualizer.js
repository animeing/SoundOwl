...
export class AudioEqualizer {
  ...
  connect(audioSource, audioContext, merger) {
    ...
    this._currentGains.forEach(element => {
      ...
      if(element == this._currentGains[0]){
        filter.type = 'lowshelf';
      } else if(element == this._currentGains[this.gains.length-1]){
    ...
    audioSource.connect(this.filters[this._currentGains[0].hz]);
    for (let i = 0; i < this._currentGains.length - 1; i++) {
      this.filters[this._currentGains[i].hz].connect(this.filters[this._currentGains[i + 1].hz]);
    }
  ...
  set gains(gains){
    ...
    this._currentGains.forEach(element => {
      if(this.filters[element.hz] == null){
        return;
      }
